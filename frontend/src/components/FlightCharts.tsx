...
const FlightCharts: React.FC<Props> = ({ result }) => {
  ...
  const renderMain = () => {
    ...
    switch (tab) {
      ...
      case 'cpcg':         return <Plot data={[line(t,c.cp,ORANGE,'CP (m)'),line(t,c.cg,BLUE,'CG (m)','dash')]} layout={{...layout('CP & CG Evolution vs Time','Time (s)','Position (m)'),legend:{...LAYOUT_BASE.legend,x:0.7,y:0.9}}} config={CFG} style={{width:'100%'}} />;
